...
const EngageWidget1: React.FC<Props> = ({ className, innerPadding = '', bgHex = '', bgColor = '', lg = '', bgImage = '' }) => {
  ...
  return (
    <div className={`card ${className}`}>
      <div className={`card-body card-rounded p-0 d-flex bg-${bgColor}`} style={{ backgroundColor: bgHex }}>
        <div className={`d-flex flex-column flex-lg-row-auto ${lg ? 'py-10 py-md-14 px-10 px-md-20 pe-lg-0' : 'p-10 p-md-20'}`}>
          ...
          <form className="d-flex flex-center py-2 px-6 bg-white rounded">
            <KTSVG path="/media/icons/duotone/General/Search.svg" className="svg-icon-1 svg-icon-primary" />
            <input
              type="text"
