              ...
              <div className="d-flex align-items-center">
                <div className="symbol symbol-35px me-4">
                  <span className={clsx('symbol-label', `bg-light-${alert.state}`)}>
                    {' '}
                    <KTSVG path={`/media/${alert.icon}`} className={`svg-icon-2 svg-icon-${alert.state}`} />
                  </span>
                </div>
        ...
        <div className="d-flex flex-column px-9">
          <div className="pt-10 pb-0">
            ...
            <div className="text-center mt-5 mb-9">
              <a href="#" className="btn btn-sm btn-primary px-6" data-bs-toggle="modal" data-bs-target="#kt_modal_upgrade_plan">
                Upgrade
              </a>
            </div>
          </div>
          ...
          {defaultLogs.map((log, index) => (
            <div key={`log${index}`} className="d-flex flex-stack py-4">
              <div className="d-flex align-items-center me-2">
                <span className={clsx('w-70px badge', `badge-light-${log.state}`, 'me-4')}>{log.code}</span>
